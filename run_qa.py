...
Major modifications are done to add support for OPT model familty and dataset pruning methods which are applied using datapruning_args.
Based on run_qa.py from the Huggingface Transformers library at commit d51aa48a76b470d95fe665bbe7ee97a3d4c21fb9.
"""
...
def main():
    ...
    if training_args.do_train:
        ...
        if datapruning_args.load_sampler is not None:
            ...
            data_sampler = torch.randperm(len(train_dataset), generator=generator)
        ...
        torch.save(
            torch.as_tensor([train_dataset['start_positions'], train_dataset['end_positions']]),
            os.path.join(training_args.output_dir, 'labels.bin')
        )
        checkpoint = None
